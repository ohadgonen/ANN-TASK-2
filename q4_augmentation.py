...
# only main change over basic CNN in Q1 - added data augmentation to the train data.
...
aug_raw_dataset = datasets.CIFAR10(root="./data", train=True, download=False)

aug_demos = [
    ("Original",        transforms.ToTensor()),
    ("Random Crop",     transforms.Compose([transforms.RandomCrop(32, padding=4), transforms.ToTensor()])),
    ("Random Rotation", transforms.Compose([transforms.RandomRotation(15), transforms.ToTensor()])),
    ("Resized Crop",    transforms.Compose([transforms.RandomResizedCrop(32, scale=(0.8, 1.0)), transforms.ToTensor()])),
    ("Horiz. Flip",     transforms.Compose([transforms.RandomHorizontalFlip(p=1.0), transforms.ToTensor()])),
...
n_cols = len(aug_demos)

fig, axes = plt.subplots(n_rows, n_cols, figsize=(n_cols * 2, n_rows * 2))

for row, idx in enumerate(sample_indices):
    pil_img, _ = aug_raw_dataset[idx]
    for col, (name, tfm) in enumerate(aug_demos):
        img = tfm(pil_img).permute(1, 2, 0).numpy()
        img = np.clip(img, 0, 1)
...
plt.suptitle("Data Augmentation Examples (CIFAR-10)", fontsize=12)
plt.tight_layout()
...
class InitialCNN(nn.Module):
    def __init__(self):
        ...
        self.bn2 = nn.BatchNorm2d(64)   # Added BatchNorm after conv2
        self.conv3 = nn.Conv2d(64, 128, 3, padding=1)
        self.bn3 = nn.BatchNorm2d(128)  # Added BatchNorm after conv3
        self.conv4 = nn.Conv2d(128, 256, 3, padding=1)
        self.bn4 = nn.BatchNorm2d(256)  # Added BatchNorm after conv4
...
for epoch in range(num_epochs):
    ...
    train_loss, train_acc, grad_norms = train(model, train_loader, optimizer)
